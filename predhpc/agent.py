...
class ResetableAgent(Agent):
    ...
    def __init__(self, Env, params={}):
        ...
        self.actual_trajectory_lengths = []
    ...
    def set_trajectory_lengths(self):
        ...
        if self.trajectory_length is not None:
            ...
            self.trajectory_length = util.get_trajectory_lengths(
                num_trajectories=self.num_trajectories, exp=self.exp, rand=self.rand
            )
    ...
    def fix_velocity_record(self, dt=None):
        ...
        self.average_measured_speed = (
            ...
            + dt / tau_speed * (np.linalg.norm(self.velocity, ord=2))
        )
    ...
    def set_pos_vel(self, pos=None, velocity=None):
        ...
        if self.Environment.dimensionality == "2D":
            ...
            self.velocity = velocity * np.array([np.cos(direction), np.sin(direction)])
    ...
    def reset(self):
        ...
        self.actual_trajectory_lengths.append(self.curr_trajectory_length)
    ...
    def log_trajectories_to_date(self):
        ...
        print(
            f"Trajectory lengths ({len(traj_leng_to_date)}) to date (in steps): {traj_leng_to_date}"
        )
    ...
    def log_trajectory_stats_to_date(self, time=True):
        ...
        print(
            f"Trajectory lengths ({len(traj_leng_to_date)}) to date: {traj_leng_to_date_mean:.2f} +/- {traj_leng_to_date_std:.2f} {traj_length_unit} each"
        )

    def plot_trajectories_to_date(self, in_minutes=True):
        ...
        traj_leng_to_date = self.get_trajectory_lengths_to_date()
        plot_util.plot_trajectory_lengths(
        ...
        for i in range(self.Environment.D):
            ...
            if n_reached:
                if self.start_pos is not None:
                    x_start = [
                        t[x]
                        for x in self.reached_reset_pos
            ...
            if plot_targets and n_targets and self.target_pos is not None:
                x_targ = [
                    t[x] for x in self.reached_target_pos if x >= startid and x < endid
                ]
        ...
        **kwargs,  # catch-all...
        ...
        if self.Environment.dimensionality == "2D":
            ...
            if plot_traj_ends == True and len(self.actual_trajectory_lengths):
                ...
                s[ends] = ms_2D * 2
                c[ends] = mcolors.to_rgba(
                    "darkred"
                )  ### set last colormap value to black
...
class TAgent(ResetableAgent, util.ParamsManagerMixin):
    ...
    def update(
        self, dt=None, speed_fact=3, drift_to_random_strength_ratio=0.7, **kwargs
    ):
        """Update the agent, optionally with a new position and velocity.
    ...
    def set_all_positions(self):
        ...
        self.target_pos = [
            T_split[i] + (edge[i] - T_split[i]) * self.target_prop for i in [0, 1]
        ]
        self.target_pos = self.set_pos(self.target_pos)
    ...
    def check_reset_pos(self, pos="both"):
        ...
        if pos == "both":
            dist = min(
                [
                    np.linalg.norm(self.pos - reset_pos, ord=2)
                    for reset_pos in self.reset_pos
                ]
            )
        elif pos == "left":
...
class BoxAgent(ResetableAgent, util.ParamsManagerMixin):
    ...
    def set_curr_target(self):
        ...
        objects, obj_types, obj_weights, obj_names = self.get_targets_and_probs(
            self.target_pos
        )
    ...
    def set_all_positions(self, first=True):
        """Set all the positions for the agent."""
    ...
    def reset(self):
        ...
        self.set_all_positions(first=False)

        if (
            len(self.reached_target_pos) == 0
            or self.num_steps_total != self.reached_target_pos[-1]
        ):
    ...
    def get_teleport_vector(self, teleport_pair, direction="in"):
        ...
        marker = self.Environment.get_teleport_pair_marker(
            teleport_pair, direction=direction
        )
        ...
        while out_coords is None:
            ...
            for x in [1, -1]:
                for y in [1, -1]:
                    ...
                    check = self._check_teleport_activate(
                        teleport_vector, check_value=check_coords
                    )
                    if check:
    ...
    def plot_trajectory(self, target_alpha=0.7, **kwargs):
        fig, ax = super().plot_trajectory(target_alpha=target_alpha, **kwargs)

        return fig, ax

    def plot_trajectory_targets(
        self, fig=None, ax=None, alpha=0.8, plot_env=True, **kwargs
    ):
    ...
    def plot_trajectory_targets_over_time(
        self, t_start=0, t_end=None, fig=None, ax=None, **kwargs
    ):
